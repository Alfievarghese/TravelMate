...
const travelData = [
  ...
  // --- ADDED DATA: New route from Kochi to Kolkata to fix the "bug" ---
  {
...
// --- STYLES (from App.css) ---
// This would typically be in a separate App.css file.
// For this single-file component, styles are managed directly.
const styles = `
...
const SearchForm = ({ handleSearch }) => {
  // --- UPDATED: Default search values changed to the new route ---
...
export default function App() {
  // State for search results and the last query made
  const [searchResults, setSearchResults] = useState([]);
  const [searchQuery, setSearchQuery] = useState({ from: '', to: '' });

  /**
   * Handles the search logic.
   * Finds a matching route in the mock data and updates the results state.
   * @param {string} from - The departure location.
   * @param {string} to - The arrival location.
   */
  const handleSearch = (from, to) => {
    // Update the search query state for potential future use (e.g., displaying "Results for...")
    setSearchQuery({ from, to });

    // Find the route in the data, case-insensitively
    ...
    // Update results: set to the route's options if found, otherwise set to an empty array
    if (matchingRoute) {
  ...
  // --- UPDATED: Perform an initial search for the new route on component mount ---
  React.useEffect(() => {
